-- ═══════════════════════════════════════════════════
-- COMBINED: All fixes - RLS, Live, Archive, Increment
-- Run this SINGLE file in Supabase SQL Editor
-- ═══════════════════════════════════════════════════

-- ── 1. Live streaming permission ──
ALTER TABLE institutes ADD COLUMN IF NOT EXISTS live_enabled BOOLEAN DEFAULT false;

-- ── 2. Content visibility & archive ──
...
-- ── 3. Atomic increment for material buyers ──
...
-- ── 4. Fix RLS policies for new tables ──

-- academic_years
DROP POLICY IF EXISTS ay_permissive ON academic_years;
DROP POLICY IF EXISTS ay_read ON academic_years;
DROP POLICY IF EXISTS ay_write ON academic_years;
...
DROP POLICY IF EXISTS eh_read ON enrollment_history;
DROP POLICY IF EXISTS eh_write ON enrollment_history;
...
DROP POLICY IF EXISTS sc_read ON student_classes;
DROP POLICY IF EXISTS sc_write ON student_classes;
...
DROP POLICY IF EXISTS ta_read ON teacher_assignments;
DROP POLICY IF EXISTS ta_write ON teacher_assignments;

// ParentCalendar — read-only academic calendar for the selected child's
// institute. Same UX pattern as (student)/calendar.tsx — institute admin
// posts events, parents see upcoming/all/past with category filtering.
...
import {
  getChildCalendarEvents, type ChildCalendarEvent,
...
export default function ParentCalendar() {
  ...
  const [events, setEvents] = useState<ChildCalendarEvent[]>([]);
  const [loading, setLoading] = useState(false);
  const [refreshing, setRefreshing] = useState(false);
  const [filter, setFilter] = useState<Filter>('upcoming');

  const load = useCallback(async () => {
    if (!childInstituteId) { setEvents([]); return; }
    setLoading(true);
    try {
      const data = await getChildCalendarEvents(childInstituteId);
      setEvents(data);
    } catch (err) {
      if (__DEV__) console.error('[parent/calendar] load', err);
    } finally {
      setLoading(false);
    }
  }, [childInstituteId]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'past') return events.filter((e) => e.end_date < today)
...
function formatDate(ymd: string): string {
  try {
    return new Date(ymd).toLocaleDateString('ar-IQ', { day: 'numeric', month: 'short', year: 'numeric' });
  } catch { return ymd; }
}
...
const styles = StyleSheet.create({
  ...
  chip: { flexDirection: 'row-reverse', alignItems: 'center', gap: 5, paddingHorizontal: 14, paddingVertical: 7, borderRadius: 999, backgroundColor: tokens.surface.surface, borderWidth: 1, borderColor: tokens.border[2] },
  ...
  card: {
    ...
    borderWidth: 1, borderColor: tokens.border[2], overflow: 'hidden',
    ...tokens.shadow.xs,
  },
  colorBar: { width: 4 },

// TeacherLeaveRequest — submit a leave request + see my previous requests.
//
// Two tabs:
//   - "طلب جديد"   : form (type / start date / end date / reason)
//   - "طلباتي"     : list of my own requests with status chip
//
// Multi-tenant: requested_by is the teacher's auth.uid(); subject_id is the
// teacher themselves (this is the teacher requesting leave for themselves).
// RLS on leave_requests already enforces:
//   - INSERT: requested_by must equal auth.uid()
//   - SELECT: requester sees own + institute members
//   - UPDATE/DELETE: only admins/institute
//
// No new dependencies. Date inputs are plain TextInput with YYYY-MM-DD pattern,
// matching the existing convention in exam-schedule-builder + payroll.
...
// today as YYYY-MM-DD in local time — used as a default for start_date so the
// admin doesn't have to type today every time.
...
export default function TeacherLeaveRequest() {
  ...
  const loadMine = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Bounded columns + .limit per bandwidth rules. Filter by requested_by
      // as defense-in-depth alongside the RLS policy.
      const { data, error } = await supabase
        .from('leave_requests')
        .select('id, type, status, start_date, end_date, start_time, reason, review_notes, reviewed_at, created_at')
        .eq('requested_by', userId)
        .order('created_at', { ascending: false })
        .limit(100);
      if (error) throw error;
      setRequests((data || []) as MyRequestRow[]);
    } catch (err: any) {
      if (__DEV__) console.error('[leave-request] loadMine', err);
  ...
  const handleSubmit = async () => {
    if (!userId || !userInstituteId) {
      Alert.alert('خطأ', 'تعذّر تحديد المؤسسة — أعد الدخول');
      return;
    }
    if (!DATE_PATTERN.test(startDate)) {
      Alert.alert('تنبيه', 'صيغة تاريخ البدء غير صحيحة (YYYY-MM-DD)');
      return;
    }
    if (needsEndDate && (!endDate || !DATE_PATTERN.test(endDate))) {
      Alert.alert('تنبيه', 'تاريخ النهاية مطلوب للإجازة متعددة الأيام');
      return;
    }
    if (needsEndDate && endDate < startDate) {
      Alert.alert('تنبيه', 'تاريخ النهاية لا يمكن أن يسبق تاريخ البدء');
      return;
    }
    if (needsStartTime && !startTime.trim()) {
      Alert.alert('تنبيه', 'وقت الانصراف مطلوب (مثال: 11:30)');
      return;
    }
    if (!reason.trim()) {
      Alert.alert('تنبيه', 'يرجى ذكر السبب باختصار');
      return;
    }
    ...
    try {
      const { error } = await supabase.from('leave_requests').insert({
        ...
        // The leave is FOR the teacher themselves — subject = self.
        subject_id: userId,
        subject_type: 'teacher',
        subject_name: userName || 'أستاذ',
        type,
      ...
      if (error) throw error;
      haptics.success();
      Alert.alert('تم الإرسال', 'تم رفع طلب الإجازة — ستصلك إشعار بالقرار', [
  ...
  // Cancel my own pending request (idempotent — RLS keeps the row, just flips
  // status). Goes through update so we don't lose history.
  const handleCancel = (req: MyRequestRow) => {
    Alert.alert(
      'إلغاء الطلب',
      `هل تريد إلغاء طلب "${TYPE_LABELS[req.type]}"؟`,
      [
        { text: 'تراجع', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Note: the existing UPDATE RLS only allows admins/institute to
              // update. If this RPC isn't permitted for the teacher, surface
              // a clearer message.
              const msg = err?.message?.includes('policy')
                ? 'لا يمكن الإلغاء بعد المراجعة — تواصل مع الإدارة'
      ...
      {tab === 'new' ? (
        ...
        >
          <View style={{ paddingHorizontal: 16, paddingTop: 14, gap: 14 }}>
            {/* Type picker */}
            <View>
              <SectionLabel title="نوع الإجازة" icon="pricetag-outline" />
            ...
            <View>
              <SectionLabel
                title={needsEndDate ? 'تاريخ البدء + النهاية' : 'تاريخ الإجازة'}
            ...
            {/* Start time (only for early_leave) */}
            {needsStartTime && (
              <View>
                <SectionLabel title="وقت الانصراف" icon="time-outline" />
                <TextInput
                  value={startTime}
